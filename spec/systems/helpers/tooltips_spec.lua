...
describe("Tooltips.buildItemStatLines - equipped items comparison", function()
    ...
    describe("when showing losses (stats equipped has but item doesn't)", function()
        it("shows damage loss in red when item has no damage but equipped does", function()
            ...
            for _, line in ipairs(lines) do
                if line.text:match("Defense:") then
                    hasDefenseGain = true
                end
                if line.text:match("Damage: 10 %- 15") and line.color[1] == 0.85 then
                    hasDamageLoss = true
                end
            end
        ...
        it("shows defense loss in red when item has no defense but equipped does", function()
            ...
            for _, line in ipairs(lines) do
                if line.text:match("Damage:") and line.color[1] == 0.3 then
                    hasDamageGain = true
                end
                if line.text:match("Defense: 10") and line.color[1] == 0.85 then
                    hasDefenseLoss = true
                end
            end
    ...
    describe("when hovering equipped item", function()
        it("shows all stats in white without comparisons", function()
            ...
            assert.matches("Damage: 10 %- 15", lines[1].text)
            assert.matches("Defense: 20", lines[2].text)
            assert.matches("%+30 Health", lines[3].text)
            assert.matches("%+10%.0%% Crit Chance", lines[4].text)
        end)
    end)
    ...
    describe("edge cases", function()
        ...
        it("handles item with no stats", function()
            ...
            assert.equal(1, #lines)
            assert.equal("No bonuses", lines[1].text)
        end)
        ...
        it("handles empty equipped items array", function()
            ...
            assert.equal(1, #lines)
            assert.same({ 0.3, 0.85, 0.4, 1 }, lines[1].color) -- Green
        end)
    end)
end)
